...
const MAX_LTV: Record<VehicleCondition, Record<string, number>> = {
...
export function getCreditTier(score: number): string {
  ...
  if (score >= 680) return 'prime';
  if (score >= 620) return 'nearprime';
  return 'subprime';
}

export function calculateAutoLtv(
  loanAmount: number,
  vehicleValue: number,
  creditScore: number,
...
export function calculateAutoPayment(loan: AutoLoan): AutoPayment {
...
export function getRecommendedTerm(vehicleAge: number, creditScore: number): number[] {
  const maxTerm = vehicleAge === 0 ? 84 : Math.max(36, 84 - (vehicleAge * 12));
  const tier = getCreditTier(creditScore);
  
  const terms: number[] = [];
  if (maxTerm >= 36) terms.push(36);
  if (maxTerm >= 48) terms.push(48);
  if (maxTerm >= 60) terms.push(60);
  if (maxTerm >= 72 && tier !== 'subprime') terms.push(72);
  if (maxTerm >= 84 && tier === 'superprime') terms.push(84);

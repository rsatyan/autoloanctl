...
function createPaymentCommand() {
    return new commander_1.Command('payment')
        ...
        if (options.compare) {
            const terms = (0, calculations_1.getRecommendedTerm)(parseInt(options.vehicleAge), parseInt(options.creditScore));
            console.log('═══════════════════════════════════════════════════════════════');
            ...
            for (const term of terms) {
                const result = (0, calculations_1.calculateAutoPayment)({
                    loanAmount: amount,
                    vehicleValue: amount,
                    interestRate: rate,
                    termMonths: term,
                ...
                console.log(`${term.toString().padEnd(10)} $${result.monthlyPayment.toLocaleString().padStart(8)}     ` +
